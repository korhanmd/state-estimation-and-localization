...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    error = K_k@(y_k.reshape(3) - p_check)

    # 3.3 Correct predicted state
    p_hat = p_check + error[:3]
    v_hat = v_check + error[3:6]
    q_hat = Quaternion(euler=error[6:]).quat_mult_right(q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_k = p_est[k-1] + delta_t*v_est[k-1] + (delta_t**2/2)*(C_ns@imu_f.data[k-1] + g)
    v_k = v_est[k-1] + delta_t*(C_ns@imu_f.data[k-1] + g)
    q_k = Quaternion(euler=(imu_w.data[k-1]*delta_t)).quat_mult_right(q_est[k-1])
    ...
    Q_k[3:, 3:] = np.identity(3)*var_imu_w*delta_t**2

    # 2. Propagate uncertainty
    P_check = F_k@p_cov[k-1]@F_k.T + l_jac@Q_k@l_jac.T

    # 3. Check availability of GNSS and LIDAR measurements
    if(any(gnss.t == imu_f.t[k])):
        gnss_index = np.where(gnss.t == imu_f.t[k])
        p_k, v_k, q_k, P_check = measurement_update(var_gnss, P_check, gnss.data[gnss_index], p_k, v_k, q_k)

    if(any(lidar.t == imu_f.t[k])):
        lidar_index = np.where(lidar.t == imu_f.t[k])
        p_k, v_k, q_k, P_check = measurement_update(var_lidar, P_check, lidar.data[lidar_index], p_k, v_k, q_k)
